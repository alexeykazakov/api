...
generate-crds: vendor prepare-host-operator prepare-member-operator
	...
	@for crd in $(HOST_CLUSTER_CRDS) ; do \
		mv deploy/crds/$(API_FULL_GROUPNAME)_$${crd}s.yaml ../host-operator/deploy/crds/$(API_GROUPNAME)_$(API_VERSION)_$${crd}_crd.yaml ; \
	done
	@for crd in $(MEMBER_CLUSTER_CRDS) ; do \
		mv deploy/crds/$(API_FULL_GROUPNAME)_$${crd}s.yaml ../member-operator/deploy/crds/$(API_GROUPNAME)_$(API_VERSION)_$${crd}_crd.yaml ; \
	done
